...
<div class="space-y-6">
    ...
    <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <form method="GET" class="flex gap-2">
            <input type="text" name="search" value="{{ request('search') }}" placeholder="Cari nama, NISN, email..."
        ...
        <a href="{{ route('admin.siswa.create') }}" class="inline-flex items-center px-4 py-2 bg-primary-600 text-white rounded-lg hover:bg-primary-700 transition-colors">
            ...
            <table class="w-full">
                <thead class="bg-gray-50">
                    <tr>
                        ...
                        <th class="px-6 py-4 text-left text-xs font-semibold text-gray-500 uppercase">Tempat PKL</th>
                ...
                <tbody class="divide-y divide-gray-50">
                    ...
                    <tr class="hover:bg-gray-50">
                        <td class="px-6 py-4">
                            <div class="flex items-center">
                                <div class="w-10 h-10 rounded-full bg-primary-100 flex items-center justify-center overflow-hidden">
                                    @if($s->foto_profil)
                                    <img src="{{ $s->foto_profil_url }}" class="w-full h-full object-cover">
                                    @else
                                    <span class="text-primary-600 font-semibold text-sm">{{ $s->inisial }}</span>
                                    @endif
                                </div>
                                <div class="ml-3">
                        ...
                        <td class="px-6 py-4">
                            <span class="text-sm text-gray-600">{{ $s->nisn }}</span>
                        </td>
                        <td class="px-6 py-4">
                            <span class="text-sm text-gray-600">{{ $s->jurusan ?? '-' }}</span>
                        </td>
                        <td class="px-6s py-4">
                            <span class="text-sm text-gray-600">
                                {{ $s->lokasi->nama_tempat_pkl ?? '-' }}
                            </span>
                        </td>
                        ...
                        <td class="px-6 py-4">
                            <div class="flex items-center justify-center gap-2">
                                <a href="{{ route('admin.siswa.show', $s->id) }}" class="p-2 text-gray-500 hover:text-primary-600 hover:bg-primary-50 rounded-lg" title="Detail">
                                    <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 12a3 3 0 11-6 0 3 3 0 016 0z" />
                                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M2.458 12C3.732 7.943 7.523 5 12 5c4.478 0 8.268 2.943 9.542 7-1.274 4.057-5.064 7-9.542 7-4.477 0-8.268-2.943-9.542-7z" />
                                    </svg>
                                </a>
                                <a href="{{ route('admin.siswa.edit', $s->id) }}" class="p-2 text-gray-500 hover:text-yellow-600 hover:bg-yellow-50 rounded-lg" title="Edit">
                                    <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
