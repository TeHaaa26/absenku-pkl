        ...
        <div class="bg-white rounded-2xl shadow-sm border border-gray-100 p-6">
            <div class="text-center">
                <div class="w-24 h-24 mx-auto rounded-full bg-primary-100 flex items-center justify-center overflow-hidden">
                    @if($guru->foto_profil)
                        <img src="{{ $guru->foto_profil_url }}" class="w-full h-full object-cover">
                    @else
                        <span class="text-primary-600 text-3xl font-bold">{{ $guru->inisial }}</span>
                    @endif
                </div>
                <h3 class="text-xl font-bold text-gray-800 mt-4">{{ $guru->nama }}</h3>
                <p class="text-gray-500">{{ $guru->nip }}</p>
            ...
            <div class="mt-6 pt-6 border-t border-gray-100 space-y-3">
                <div class="flex justify-between">
                    ...
                    <span class="text-gray-800">{{ $guru->email }}</span>
                </div>
                <div class="flex justify-between">
                    <span class="text-gray-500">Telepon</span>
                    <span class="text-gray-800">{{ $guru->no_telepon ?? '-' }}</span>
                </div>
                <div class="flex justify-between">
                    <span class="text-gray-500">Jabatan</span>
                    <span class="text-gray-800">{{ $guru->jabatan ?? '-' }}</span>
                </div>
                <div class="flex justify-between">
                    <span class="text-gray-500">Jenis Kelamin</span>
                    <span class="text-gray-800">{{ $guru->jenis_kelamin == 'L' ? 'Laki-laki' : ($guru->jenis_kelamin == 'P' ? 'Perempuan' : '-') }}</span>
                </div>
            </div>
            
            <div class="mt-6 pt-6 border-t border-gray-100">
                <a href="{{ route('admin.guru.edit', $guru->id) }}" class="block w-full text-center px-4 py-2 bg-primary-600 text-white rounded-lg hover:bg-primary-700 transition-colors">
                    Edit Data
                </a>
            </div>
        </div>
